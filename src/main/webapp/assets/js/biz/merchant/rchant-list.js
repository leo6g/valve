...
var global_limit = 10 ;
	...
	queryMerchantState();
	//初始化上传按钮
	getUpload("uploadIcon","imagePath","picIcon");
	//初始化上传按钮
	getUpload("edit-uploadIcon","edit-imagePath","edit-picIcon");
		...
		Util.ajax.postJson(url, params, function(data, flag){
			if(flag && data.returnCode=="0"){
				...
				if(viewinfo !=null)
				{
					...
					var url = contextPath +"/"+ viewinfo.imagePath;
					if(viewinfo.imagePath!='' && viewinfo.imagePath!=null){
						$("#edit-picIcon").attr("src",url);
					}
