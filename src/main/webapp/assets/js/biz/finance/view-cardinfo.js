...
$(document).ready(function(){
		...
		showBtn();
		//新增审核意见信息信息
		$("#saveBIZAuthInfoBtn").click(function(){
			addBIZAuthInfo();
		});
		...
		function initBIZAuthInfoListInfo(currentPage, limit){
			...
			var params = "prodId="+cardId+"&pageNumber="+currentPage+"&limit="+limit;
			//异步请求信用卡信息发布表列表数据
			Util.ajax.postJson(url, params, function(datas, flag){
				...
				if(flag && datas.returnCode=="0"){
					...
					//initPaginator(currentPage, datas.bean.count, initBIZAuthInfoListInfo);
				}
			});
		};
	//动态替换审批详情审批状态
...
////////////////////////////////////////////////////////////	
	...
	//初始化菜单信息
	function queryCardById(){
		var cardId = $("#cardId").val();
		var url = contextPath + "/biz/cardinfo/getById?cardId="+cardId;
		var params = "";
		//异步请求职位列表数据
	...
	//返回信用卡管理页面
	function backHome(){
		C.load(contextPath + "/biz/cardinfo/list");
	}
	
	//修改信用卡表状态
	function updateWXCardInfoState(authState,cardId){
		var url=contextPath+"/biz/cardinfo/updateWXCardInfo";
		var formdata = "authState="+authState+"&cardId="+cardId;
	...
	function showBtn(){
		...
		var authState = $("#authState").val();
		 /*if(role == 'business'){
			 $("#bIZAuthInfoDiv").css('display','none');
